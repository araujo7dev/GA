    ...
    <div class="container w-auto text-dark bg-white border border-dark p-1 rounded" style="box-shadow: 2px 2px 25px black;">
        ...
        <?php 
            ...
            if(is_numeric($idLocador) and is_numeric($idLocatario) and is_numeric($idImovel)){
                ...
                $queryLocador = $pdo->query("SELECT id, nome, est_civil, profissao, cpf FROM locats WHERE   id='$idLocador'");

                $locador = $queryLocador->fetch(); //mysqli_fetch_assoc($queryLocador);

                // Query na tabela locatário
                $queryLocatario = $pdo->query("SELECT id, nome, est_civil, profissao, cpf FROM locats WHERE id='$idLocatario'");

                $locatario = $queryLocatario->fetch();

                // Query na tabela imóvel
                $status = 0; // Status 0 - Não alugado
                $queryImovel = $pdo->query("SELECT id, descricao, utilizacao, wc, area, garagem, designacao, logradouro, numero, complemento, bairro, cidade, cep FROM imoveis WHERE   id='$idImovel' AND status='$status'");
